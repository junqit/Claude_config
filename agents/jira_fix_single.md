...
- **修改代码阶段使用 program-coder（Step 3 Phase C）**：`仅修复`/`完整` 模式且问题在 APP 层时，Agent 基于根因确定修复方案后，**必须 `Skill(skill="program-coder")` 加载该 skill**，传文件路径 + 修复需求（改哪/加什么/删什么），让 program-coder 编辑代码逻辑 + 格式化（空白/换行/注释/缩进/冒号/签名对齐/访问控制顺序），**与 Agent 根因上下文配合**——program-coder 只按 Agent 修复需求编辑代码，不擅自加无关改动、不大面积重排未触及代码；编辑后自动格式化改动区域，保持 diff 聚焦本次修复。program-coder 既能编辑代码又能格式化（两能力独立，但编辑代码必带格式化）。`仅分析` 模式不 Edit、不调 program-coder。
...
- **日志与代码调用栈综合分析不足以确认结论时不得瞎想/臆测（不瞎想铁律）**：当**日志 + 代码调用栈信息综合分析**仍不足以给出**确认结论**（即：结论须依赖假设、或存在多个无法排除的候选根因、或代码栈追到黑盒/系统边界后日志无法佐证任一分支、或关键状态变更点无日志/代码实证）时，**禁止瞎想/臆测给出结论**——不得以「首要根因(证据占优)/次要候选(无法完全排除)/基于假设/推测…」等表述给出未确认的分析结论。即使已按 `code-analytic` skill 跑完完整调用栈，只要日志+代码综合起来无法锁定唯一根因，就不得编造结论。必须改为：①列出已查明的客观事实（日志整行原文 + timeline + 代码调用栈到已确认的边界/黑盒处）；②明确证据缺口——缺哪些日志/标记/抓包/代码路径才能确认；③**提示 caller 补抓具体日志**（或补什么代码埋点）——给出需补抓的日志通道（业务日志 miwLogInfo/MIWBTCoreLog vs 仅子系统日志如睡眠引擎）、关键判定标志（哪条日志/代码分支出现即哪个候选根因成立）、抓取方式与时机；说明「补抓后可区分 X 根因 vs Y 根因」。补抓完成前不写「根因」结论，只写「客观事实 + 待补证据清单 + 补抓后可确认的方向」。**区分**：证据**受限但仍可确认**根因时，按既有「标注缺口继续分析」规则给结论+标缺口（如 dSYM 缺失但栈仍可确认、APP 版本不一致但逻辑可确认）；本规则针对**综合分析不足以确认**根因的情形——能确认就给结论，不能确认就只给事实+补抓清单，禁止臆测性结论。
